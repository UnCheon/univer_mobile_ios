...
- (void) layoutSubviews
{
    ...
    self.textLabel.frame = CGRectMake(10, 8, 200, 20);
	...
	if(self.badgeString)
	{
		...
		if (version <= 3.0)
		{
			badgeframe = CGRectMake(self.contentView.frame.size.width - (badgeSize.width+16), (CGFloat)round((self.contentView.frame.size.height ) / 2), badgeSize.width+16, 18);
		}
		else
		{
			badgeframe = CGRectMake(self.contentView.frame.size.width - (badgeSize.width+16) - 10, (CGFloat)round((self.contentView.frame.size.height ) / 2), badgeSize.width+16, 18);
		}
